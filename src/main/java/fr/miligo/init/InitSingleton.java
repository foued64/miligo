...
import fr.miligo.model.entities.parc.ConfigurationBorne;
...
import fr.miligo.model.facades.parc.FacadeConfigurationBorne;
...
public class InitSingleton {
	...
	@Inject
	private FacadeConfigurationBorne facadeConfigurationBorne;
	...
	public void init() {
		if (log.isInfoEnabled()) {
			log.info("Démarrage de l'application : Miligo");
		}
	...
	private void initialiserGsbdd() {
		if (facadeGsbdd.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des gsbdd.");
			}
			...
			try {
				...
				log.error("Impossible de charger gsbdd.properties" + e.getMessage());
	...
	private void initialiserVille() {
		if (facadeVille.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des villes.");
			}
			...
			try {
				...
				log.error("Impossible de charger ville.properties" + e.getMessage());
	...
	private void initialiserAdresse() {
		if (facadeAdresse.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des adresses.");
			}
			...
			try {
				...
				// boucle lambda de creation de données
				adresseProperties.entrySet().stream().map(this::mapPropertyEntryToAdresse)
						.forEach(facadeAdresse::create);
			} catch (Exception e) {
				log.error("Impossible de charger adresse.properties" + e.getMessage());
	...
	private void initialiserMarque() {
		if (facadeMarque.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des marques.");
			}
			...
			try {
				...
				log.error("Impossible de charger marque.properties" + e.getMessage());
	...
	private void initialiserModele() {
		if (facadeModele.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des modeles.");
			}
			...
			try {
				...
				log.error("Impossible de charger modele.properties" + e.getMessage());
	...
	private void initialiserTypeVehicule() {
		if (facadeTypeVehicule.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des types de véhicules");
			}
			...
			try {
				...
				// boucle lambda de creation de données
				typeVehiculeProperties.entrySet().stream().map(this::mapPropertyEntryToTypeVehicule)
						.forEach(facadeTypeVehicule::create);
			} catch (Exception e) {
				log.error("Impossible de charger type_vehicule.properties" + e.getMessage());
	...
	private void initialiserSite() {
		if (facadeSite.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des sites.");
			}
			...
			try {
				...
				log.error("Impossible de charger site.properties" + e.getMessage());
	...
	private void initialiserBorne() {
		if (facadeBorne.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des bornes.");
			}
			...
			try {
				...
				for (Borne borne : facadeBorne.readAll()) {
					ConfigurationBorne configurationBorne = facadeConfigurationBorne.newInstance();
					configurationBorne.setDistanceMax(16);
					configurationBorne.setDistanceMin(8);
					configurationBorne.setSeuilRechargement(50);
					configurationBorne.setTempsCharge(new SimpleDateFormat("HH:mm:ss").parse("01:00:00"));
					borne.setConfigurationBorne(configurationBorne);
					facadeBorne.update(borne);
				}
			} catch (Exception e) {
				if (log.isErrorEnabled()) {
					log.error("Impossible de charger borne.properties" + e.getMessage());
				}
			}
		}
	}
	...
	private void initialiserTrajet() {
		if (facadeTrajet.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des trajets.");
			}
			...
			try {
				...
				log.error("Impossible de charger trajet.properties" + e.getMessage());
	...
	public Vehicule mapPropertyEntryToVehicule(Entry<?, ?> entry) {
		...
		return facadeVehicule.newInstance(libelle, immatriculation, kilometrage, batterie, puissance,
				dateMiseEnCirculation, borne, disponible, modele);
	}
	...
	private void initialiserVehicule() {
		if (facadeVehicule.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des véhicules.");
			}
			...
			try {
				...
				// boucle lambda de creation de données
				vehiculeProperties.entrySet().stream().map(this::mapPropertyEntryToVehicule)
						.forEach(facadeVehicule::create);
			} catch (Exception e) {
				log.error("Impossible de charger vehicule.properties" + e.getMessage());
	...
	private void initialiserMaintenance() {
		if (facadeMaintenance.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des maintenances.");
			}
			...
			try {
				...
				// boucle lambda de creation de données
				maintenanceProperties.entrySet().stream().map(this::mapPropertyEntryToMaintenance)
						.forEach(facadeMaintenance::create);
			} catch (Exception e) {
				log.error("Impossible de charger maintenance.properties" + e.getMessage());
	...
	private void initialiserClient() {
		if (facadeClient.isEmpty()) {
			if (log.isInfoEnabled()) {
				log.info("Initialisation de la liste des clients.");
			}
			...
			try {
				...
				log.error("Impossible de charger client.properties" + e.getMessage());
	...
	private static Properties loadFromResource(String ressourceName) {
		try (InputStream in = InitSingleton.class.getClassLoader().getResourceAsStream(ressourceName)) {
			...
			log.error(String.format("Impossible de lire le fichier %s", ressourceName));
